...
export default function UserProfile() {
  ...
  useEffect(() => {
    ...
    fetchFollowings();
    fetchUsername();
  }, [user, user.id, following, userData])
  ...
  const fetchUsername = async () => {
    ...
    if (!data || data.length === 0) {
      console.error('Current user username not found for id:', user.id);
      return; 
    }
  ...
  const sendEmail = async () => {
    ...
    .insert([{
      user_id: user.id, 
      interacter_id: userData.user_id, 
      user_username: currentUsername, 
      interacter_username: userData.username, 
      profile_link: currentUsername,
